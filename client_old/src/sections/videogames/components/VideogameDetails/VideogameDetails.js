...
const VideogameDetails = (props) => {
  ...
  const videogameQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        ...response.data,
        [PROPERTIES.developers]: response.data[PROPERTIES.developers].length
          ? response.data[PROPERTIES.developers]
          : [{}],
        [PROPERTIES.publishers]: response.data[PROPERTIES.publishers].length
          ? response.data[PROPERTIES.publishers]
          : [{}],
        [PROPERTIES.releaseDates]: response.data[PROPERTIES.releaseDates].length
          ? response.data[PROPERTIES.releaseDates]
          : [{}],
      };
    },
    queryKey: ["videogame", value.id],
  });
  ...
  const snakeCase = (string = "", separator = "-") => {
    return string
      .replace(/\W+/g, " ")
      .split(/ |\B(?=[A-Z])/)
  ...
  return (
    <>
      ...
      {videogameQuery.data.title && (
        <StyledImage
          alt={`${snakeCase(videogameQuery.data.title)}-cover-art`}
          height={400}
          src={`${api.hostname}/static/${snakeCase(
            videogameQuery.data.title
          )}.png`}
        />
      )}
